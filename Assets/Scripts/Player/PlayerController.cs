...
public class PlayerController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else if(drillLine.enabled)
        {
            drillLine.enabled = false;
        }
    ...
    private void Drill()
    {
        RaycastHit[] colliders = Physics.RaycastAll(mainCamera.transform.position, mainCamera.transform.forward, drillRange);
        if(colliders.Length > 0) drillLine.enabled = true;
        foreach (RaycastHit collider in colliders)
        {
            if (collider.collider.isTrigger || collider.collider.CompareTag("Player")) continue;
            drillLine.SetPositions(new Vector3[] { drillPoint.position, collider.point });
            if (collider.transform.TryGetComponent(out Resource resource))
            {
                resource.TakeDamage(drillDamage * Time.deltaTime);
            }
            break;
        }
    }
